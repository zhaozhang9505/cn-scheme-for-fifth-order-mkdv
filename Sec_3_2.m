...
U =25*(3*exp(7/2*x-40)+2*exp(4*x-35)+75*exp(-10+3/2*x)+50*exp(-15+x))./(625*exp(-20+3*x)+1200*exp(5/2*x-25)+625*exp(-30+2*x)+exp(5*x-50)+625);
...
for n = 1:Nt
    ...
    Uex=25*(3*exp(7/2*x-307/32*t(n)-40)+2*exp(4*x-259/16*t(n)-35)...
        +75*exp(-243/32*t(n)+3/2*x-10)+50*exp(-t(n)+x-15))./(625*exp(-243/16*t(n)+3*x-20)...
        +1200*exp(5/2*x-275/32*t(n)-25)+625*exp(-2*t(n)+2*x-30)...
        +exp(5*x-275/16*t(n)-50)+625);
    Uex(1:3)   = 0;
    Uex(L-2:L) = 0;
